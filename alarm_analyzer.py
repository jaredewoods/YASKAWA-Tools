...
error_dict = {
    "0020": {
        "0001": {
            ...
            "Signal of Defect": "Not specified",
    ...
    "0021": {
        "0032": {
            ...
            "Sub-Code Description": "No description available"
        }
    },
    "0030": {
        "0032": {
            "Message": "ROM ERROR",
            "Location of Defect": "Not specified",
            "Signal of Defect": "Not specified",
    ...
    "4400": {
        "0001:0006": {
            ...
            "Sub-Code Description": "The sub code stands for the alarm factor. H249"
        }
    },
    "4401": {
        "0001:006A": {
...
class ErrorLookupApp:
    def __init__(self, master):
        ...
        # Alarm Code ComboBox
        self.alarm_label = ttk.Label(master, text="Alarm Code:")
        self.alarm_label.grid(row=0, column=0)
        self.alarm_combo = ttk.Combobox(master, values=list(error_dict.keys()))
        self.alarm_combo.grid(row=0, column=1)
        self.alarm_combo.bind('<<ComboboxSelected>>', self.update_subcodes)

        # Sub-Code ComboBox
        self.subcode_label = ttk.Label(master, text="Sub-Code:")
        self.subcode_label.grid(row=1, column=0)
        self.subcode_combo = ttk.Combobox(master)
        self.subcode_combo.grid(row=1, column=1)

        # Display Area
        self.info_text = tk.Text(master, height=15, width=80)
    ...
    def display_info(self, event):
        ...
        # Always display message, check for empty content
        message = details.get('Message', '')
        if message:
            self.info_text.insert(tk.END, f"Message: {message}\n")

        # Conditional checks for Location and Signal
        location = details.get('Location', '')
        if location:
            self.info_text.insert(tk.END, f"Location: {location}\n")

        signal = details.get('Signal', '')
        if signal:
            self.info_text.insert(tk.END, f"Signal: {signal}\n")

        # Always display Cause if it exists
        cause = details.get('Cause', '')
        if cause:
            self.info_text.insert(tk.END, f"Cause: {cause}\n")

        # Display Potential Causes only if they exist and are non-generic
        potential_causes = ', '.join(details.get('Potential Causes', []))
        if potential_causes:
            self.info_text.insert(tk.END, f"Potential Causes: {potential_causes}\n")

        # Sub-Code Description only if it is meaningful and not a known placeholder
        sub_code_description = details.get('Sub-Code Description', '')
        if sub_code_description and not sub_code_description.lower().startswith("no description available"):
            self.info_text.insert(tk.END, f"Sub-Code Description: {sub_code_description}")
